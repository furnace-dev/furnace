...
fn copy_string_to_buffer(
    s: String, buff: UnsafePointer[ffi.c_char], buff_size: ffi.c_int
) -> Int:
    """
    将字符串复制到结果缓冲区中.
...
fn main() raises:
    ...
    var mojo_tv_init = get_wrapper(
        _tv_init, "c_int", "c_char_p", "c_char_p", "c_char_p", "c_int"
    )
    _ffi.set_tv_init(mojo_tv_init)

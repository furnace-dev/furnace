...
fn sha256_encode(
    byte_view: UnsafePointer[UInt8], length: Int
) -> InlinedFixedVector[UInt8, 32]:
    ...
    for chunk_number in range(exact_chunks):
        ...
        for dword_i in range(16):
            ...
            var start_byte_overall = start_byte_within_chunk + (
                64 * chunk_number
            )
        ...
        for i in range(16, 64):
            ...
            var s0 = bitrr(w[i - 15], 7) ^ bitrr(w[i - 15], 18) ^ (
                w[i - 15] >> 3
            )
            # s1 := (w[i-2] rightrotate 17) xor (w[i-2] rightrotate 19) xor (w[i-2] rightshift 10)
            var s1 = bitrr(w[i - 2], 17) ^ bitrr(w[i - 2], 19) ^ (
                w[i - 2] >> 10
            )
